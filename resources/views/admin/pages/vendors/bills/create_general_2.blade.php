...
@section('content')
            ...
            <div class="card-body">
                <form action="{{ route('vendors.bills.general_store') }}" method="POST" id="billForm">
                    ...
                    <div class="row g-4 mb-5">
                        <div class="col-md-4">
                            <label class="form-label fw-semibold" for="vendor_id">
                    ...
                    <div class="row g-4 mb-3">
                        ...
                        <div class="col-md-6">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <span class="badge bg-label-warning rounded-pill" id="extra-charges-count">0 Charges</span>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <span class="badge bg-label-success rounded-pill" id="additional-charges-count">0 Charges</span>
                            </div>
                            <div id="additional-charges-container" class="mb-3">
                                <!-- Additional charges will be added here -->
                            </div>
                            <button type="button" class="btn btn-outline-success" id="add-additional-charge">
                    ...
                    <div class="row">
                        <div class="col-md-5 col-lg-4 ms-auto">
                            <div class="card bg-light border-0 shadow-sm">
                                <div class="card-body">
                                    <h6 class="card-title mb-3 text-muted">Bill Summary</h6>
                                    ...
                                    <div class="d-flex justify-content-between mb-3">
                                        <span class="text-muted">Additional Charges (Add):</span>
                                        <span class="fw-semibold text-success" id="additional-charges-total-display">PKR 0.00</span>
                                    </div>
                    ...
                    <div class="row mt-4 pt-3">
                        <div class="col-12 d-flex justify-content-end gap-2">
                            <button type="submit" class="btn btn-primary px-4" id="submitBtn">
                                <i class="bx bx-save me-1"></i> Create Bill (Pending Approval)
                            </button>
                            <a href="{{ route('vendors.list') }}" class="btn btn-outline-secondary px-4">
    ...
    <template id="product-row-template">
        <div class="product-row card mb-3 border">
            <div class="card-body p-3">
                <input type="hidden" name="products[INDEX][product_type]" value="existing">
                
                <div class="row g-3">
                    <!-- Row 1: Product, Description, Qty, Packing -->
                    <div class="col-12">
                        <div class="row g-3">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-2">
                                <label class="form-label small fw-semibold">Packing (KG)</label>
                                <input type="number" step="0.01" class="form-control form-control-sm packing-input" name="products[INDEX][packing]" placeholder="KG" />
                            </div>
                    ...
                    <!-- Row 2: Bardana Wt, Net Weight, Rate, Line Total, Remove Button -->
                    <div class="col-12">
                        <div class="row g-3">
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number" step="0.01" class="form-control form-control-sm rate-input" name="products[INDEX][price]" placeholder="0.00" />
                            </div>

                            <div class="col-md-2">
                                <label class="form-label small fw-semibold">Line Total</label>
                                <input type="hidden" class="total-raw" name="products[INDEX][total_price]" />
    ...
    <!-- Extra Charge Template (Subtract) -->
    <template id="extra-charge-template">
        <div class="extra-charge-row card mb-2 border">
            <div class="card-body py-2 px-3">
                <div class="row g-3 align-items-end">
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold">Deduction Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control form-control-sm" name="extra_charges[INDEX][name]" placeholder="e.g., Labour, Freight" required />
                    </div>
                    <div class="col-md-4">
    ...
    <!-- Additional Charge Template (Add) -->
    <template id="additional-charge-template">
        <div class="additional-charge-row card mb-2 border">
            <div class="card-body py-2 px-3">
                <div class="row g-3 align-items-end">
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold">Charge Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control form-control-sm" name="additional_charges[INDEX][name]" placeholder="e.g., Delivery, Service" required />
                    </div>
                    <div class="col-md-4">
                        <label class="form-label small fw-semibold">Amount (PKR)</label>
                        <input type="number" step="0.01" class="form-control form-control-sm additional-amount-input" name="additional_charges[INDEX][amount]" placeholder="0.00" required />
                    </div>
                    <div class="col-md-2">
                        <button type="button" class="btn btn-sm btn-danger w-100 remove-additional-charge">
                            <i class="bx bx-trash"></i> Remove
                        </button>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateLineTotal(productRow) {
                ...
                const rate = parseFloat(productRow.find('.rate-input').val()) || 0;
            ...
            function calculateTotals() {
                ...
                $('.additional-charge-row').each(function() {
                    const amount = parseFloat($(this).find('.additional-amount-input').val()) || 0;
                    additionalTotal += amount;
                });
                
                const grandTotal = subtotal - extraTotal + additionalTotal;
                
                $('#subtotal-display').text('PKR ' + subtotal.toFixed(2));
                $('#extra-charges-total-display').text('PKR ' + extraTotal.toFixed(2));
                $('#additional-charges-total-display').text('PKR ' + additionalTotal.toFixed(2));
                ...
                $('#extra-charges-count').text($('.extra-charge-row').length + ' Charge' + ($('.extra-charge-row').length !== 1 ? 's' : ''));
                $('#additional-charges-count').text($('.additional-charge-row').length + ' Charge' + ($('.additional-charge-row').length !== 1 ? 's' : ''));
            }
            ...
            function addAdditionalCharge() {
                const template = $('#additional-charge-template').html();
                const row = $(template.replace(/INDEX/g, additionalChargeIndex));
                
                row.find('.additional-amount-input').on('input', function() {
                    calculateTotals();
                });
                ...
                $('#additional-charges-container').append(row);
            ...
            $('#add-additional-charge').on('click', addAdditionalCharge);
        });
    </script>
@endpush