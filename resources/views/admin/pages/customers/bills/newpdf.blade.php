                ...
                <td style="width: 52%; padding-left: 20px; vertical-align: top;">
                    <table style="width: 100%; border-collapse: collapse; min-height: 160px;">
                        <tr>
                            ...
                            <td style="background: #fff; border: 1px solid #dee2e6; padding: 8px;">
                                {{ 'SFIS' . str_pad($bill->id, 3, '0', STR_PAD_LEFT) }}
                            </td>
                        </tr>
                        <tr>
                            <td
        ...
        <table class="content-table">
            ...
            <tbody>
                ...
                @foreach ($bill->billProducts as $index => $billProduct)
                    @php
                        // Prefer stored total if available, otherwise compute from net weight and rate per 40kg
                        $ratePer40 = $billProduct->rate_per_40kg ?? 0;
                        $totalWeight = $billProduct->total_weight ?? 0;
                        $bardana = $billProduct->bardana_weight ?? 0;
                        $netWeight = $billProduct->net_weight ?? max(0, $totalWeight - $bardana);
                        $computedTotal = $netWeight * ($ratePer40 / 40);
                        $rowTotal = $billProduct->total ?? $computedTotal;
                        $subtotal += $rowTotal;
                    @endphp
                    <tr>
                        ...
                        <td class="text-right">{{ number_format($ratePer40, 2) }}</td>
                ...
                <tr style="background-color: #e8e8e8; font-weight: bold;">
                    <td colspan="9" class="text-center">NET PAYABLE</td>
